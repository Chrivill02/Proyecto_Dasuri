...
export default function AgendaPage() {
  ...
  const calcularCostoTotal = (servicios) => {
    if (!servicios || !Array.isArray(servicios)) return 0;
    return servicios.reduce((total, servicio) => total + parseFloat(servicio.precio || 0), 0).toFixed(2);
  };
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="font-medium text-purple-800">
                            Estado: <span className={`font-medium ${
                              ...
                              cita.estado === 'Completada' ? 'text-blue-600' : 'text-gray-800'
                            }`}>{cita.estado}</span>
                          </p>
                          
                          {/* Mostrar servicios con mejor formato */}
                              ...
                              <ul className="ml-6 list-disc">
                                {cita.servicios.map(servicio => (
                                  <li key={servicio.id} className="text-gray-800">
                                    {servicio.nombre_servicio} - <span className="text-amber-600 font-medium">Q{parseFloat(servicio.precio).toFixed(2)}</span>
                                  </li>
                                ))}
                              </ul>
                            </div>
                        ...
                        <div className="text-right">
                          <p className="font-bold text-amber-600 text-xl">Q{parseFloat(cita.costo).toFixed(2)}</p>
                      ...
                      {serviciosDisponibles.map(servicio => (
                        <div key={servicio.id} className="flex items-center">
                          ...
                          <label htmlFor={`servicio-${servicio.id}`} className="text-gray-700">
                            {servicio.nombre_servicio} - <span className="font-medium text-amber-600">Q{parseFloat(servicio.precio).toFixed(2)}</span>
                          </label>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mt-2 text-right">
                    <p className="text-sm text-purple-700">
                      ...
                      <span className="ml-2 font-bold text-amber-600">
                        Q{calcularCostoTotal(
                          serviciosDisponibles.filter(s => serviciosSeleccionados.includes(s.id))
                        )}
                      </span>
                    </p>
                  </div>
